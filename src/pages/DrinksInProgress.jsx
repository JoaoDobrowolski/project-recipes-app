...
function DrinksInProgress() {
  const { mealOrDrink, detailsDrink } = useContext(RecipeAppContext);
  const history = useHistory();
  const [renderAux, setRenderAux] = useState();

  const ingredients = Object.entries(detailsDrink)
    .filter((el) => (el[1] !== '' && el[0]
  ...
  return (
    <div>
      <img
        ...
        alt="foto"
        src={ detailsDrink[0][`${handleKeyObj('str')}Thumb`] }
      />
      <h1 data-testid="recipe-title">{ detailsDrink[0][handleKeyObj('str')]}</h1>
      <button data-testid="share-btn" type="button">share</button>
      <button data-testid="favorite-btn" type="button">favorite</button>
      <p data-testid="recipe-category">{detailsDrink[0].strCategory}</p>
